...
Progress so far:
- **Phase 1 (done):** fixed CTAs that pointed at dead anchors, a non-form calendar link, a wrong slug, an unlinked logo, and several `<button>` elements with no `href`/JS at all. Every "Get Audit" style CTA site-wide now points at `https://csicservices.com/get-audit/`.
- **Phase 2, step 1 (done):** `template-parts/csic-nav-links.php` is a shared partial (included via `get_template_part()`) holding the Services / Tech Stack / Blog / Get Audit nav links, now included by `header.php`, `page-csic-master.php`, and `front-page.php` instead of each hand-maintaining its own copy — this is exactly where the missing-Blog-link and wrong-Get-Audit-href bugs came from. The logo markup and CTA button (text/style differs intentionally per template: "Growth Audit" vs "Start Growing") were left inline rather than merged, since merging those would also mean merging `header.php`'s `fixed` positioning with `front-page.php`'s `sticky` positioning — a real layout change that hasn't been visually verified.
- **Decided, not yet executed:** the two competing primary colors should converge on `#2563eb` (already used by `front-page.php`/`header.php`/`page-csic-master.php`/`template-service.php`/`home.php`/`archive-blog.php`) as the primary brand color, with `#066aab` (used as `--primary` in the isolated legal/dental/SEO-growth/audit/contact/law templates) repurposed as a secondary/supporting accent rather than removed outright. Not yet applied — do this only as a deliberate, reviewed pass since it touches CTA/icon/border colors throughout those six templates.
- **Not yet touched:** CDN version pinning (Font Awesome 6.0.0 vs 6.4.2, unpinned Tailwind CDN) and footer markup — footers differ more substantially in structure between templates than the header nav did, so unifying them is a bigger design decision, not a pure dedup.
