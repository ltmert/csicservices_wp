<?php
/*
Template Name: CSIC Salesforce Integration
*/
get_header(); ?>

<main class="bg-white">
    <!-- HERO SECTION (US Region Optimizationa: Punchy, Benefit-Driven) -->
    <section class="relative pt-32 pb-24 bg-slate-50 overflow-hidden border-b border-slate-200">
        <div class="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-12 items-center">
            <div>
                <span class="inline-flex items-center px-2.5 py-1 rounded bg-blue-100 text-blue-700 text-[10px] font-black uppercase tracking-widest mb-4">
                    Enterprise Cloud Engineering
                </span>
                <h1 class="text-4xl md:text-5xl lg:text-6xl font-extrabold text-slate-900 tracking-tighter leading-tight">
                    Stop Fighting Your CRM. <br>
                    <span class="text-blue-600">Automate Your Operations.</span>
                </h1>
                <p class="text-lg text-slate-600 mt-6 mb-8 max-w-xl leading-relaxed">
                    We deploy custom-engineered Salesforce environments that integrate your core business channels—directly syncing communication, billing, and logistics pipelines without the overhead of an in-house team.
                </p>
                <div class="flex flex-col sm:flex-row gap-4">
                    <a href="#gap-analysis" class="btn-main px-10 py-5 rounded-xl font-black text-xs text-center shadow-xl shadow-blue-200">
                        Request a Technical Teardown
                    </a>
                </div>
            </div>
            <div class="relative">
                <div class="absolute inset-0 bg-blue-600/5 rounded-3xl -rotate-2 scale-105"></div>
                <div class="bg-white p-8 rounded-3xl shadow-2xl relative z-10 border border-slate-100">
                    <div class="flex items-center justify-between border-b border-slate-100 pb-4 mb-6">
                        <div class="flex items-center space-x-2">
                            <span class="w-3 h-3 rounded-full bg-red-400"></span>
                            <span class="w-3 h-3 rounded-full bg-yellow-400"></span>
                            <span class="w-3 h-3 rounded-full bg-green-400"></span>
                        </div>
                        <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">CSIC INTEGRATION MAP</span>
                    </div>
                    <ul class="space-y-4">
                        <li class="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                            <span class="text-xs font-bold text-slate-700"><i class="fas fa-file-invoice-dollar text-blue-600 mr-2"></i> Stripe Payments</span>
                            <span class="text-[9px] font-black text-green-600 bg-green-100 px-2 py-0.5 rounded">AUTO-SYNCED</span>
                        </li>
                        <li class="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                            <span class="text-xs font-bold text-slate-700"><i class="fab fa-whatsapp text-green-500 mr-2"></i> WhatsApp / SMS Notifications</span>
                            <span class="text-[9px] font-black text-blue-600 bg-blue-100 px-2 py-0.5 rounded">REALTIME</span>
                        </li>
                        <li class="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                            <span class="text-xs font-bold text-slate-700"><i class="fas fa-video text-purple-600 mr-2"></i> Zoom AI Transcribing</span>
                            <span class="text-[9px] font-black text-purple-600 bg-purple-100 px-2 py-0.5 rounded">CONNECTED</span>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
    </section>

    <!-- CORE INTEGRATIONS GRID -->
    <section class="py-24 bg-white">
        <div class="max-w-7xl mx-auto px-6">
            <div class="text-center mb-16">
                <h2 class="text-3xl md:text-4xl font-extrabold text-slate-900 tracking-tighter uppercase">Engineered Salesforce Solutions</h2>
                <p class="text-slate-500 mt-4 max-w-2xl mx-auto">We build robust API architectures that tie external services straight into your Opportunity, Contact, and Case layouts.</p>
            </div>

            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                <!-- iFax Integration -->
                <div class="p-8 border border-slate-100 bg-slate-50 rounded-2xl shadow-sm hover:shadow-lg transition">
                    <div class="w-12 h-12 bg-blue-100 text-blue-600 rounded-xl flex items-center justify-center mb-6">
                        <i class="fas fa-fax text-xl"></i>
                    </div>
                    <h3 class="font-extrabold text-lg text-slate-950 uppercase tracking-tight mb-2">iFax Automation</h3>
                    <p class="text-sm text-slate-500 leading-relaxed">Secure, inbound and outbound medical or legal fax transmissions processed directly inside standard objects. No paper, 100% cloud compliance.</p>
                </div>

                <!-- USPS Integration -->
                <div class="p-8 border border-slate-100 bg-slate-50 rounded-2xl shadow-sm hover:shadow-lg transition">
                    <div class="w-12 h-12 bg-blue-100 text-blue-600 rounded-xl flex items-center justify-center mb-6">
                        <i class="fas fa-envelope-open-text text-xl"></i>
                    </div>
                    <h3 class="font-extrabold text-lg text-slate-950 uppercase tracking-tight mb-2">USPS Logistics</h3>
                    <p class="text-sm text-slate-500 leading-relaxed">Real-time address validation, shipping tracking details, and automated physical post/direct mail dispatches directly linked to your customer data loops.</p>
                </div>

                <!-- WhatsApp & SMS Integration -->
                <div class="p-8 border border-slate-100 bg-slate-50 rounded-2xl shadow-sm hover:shadow-lg transition">
                    <div class="w-12 h-12 bg-blue-100 text-blue-600 rounded-xl flex items-center justify-center mb-6">
                        <i class="fab fa-whatsapp text-xl"></i>
                    </div>
                    <h3 class="font-extrabold text-lg text-slate-950 uppercase tracking-tight mb-2">WhatsApp & SMS</h3>
                    <p class="text-sm text-slate-500 leading-relaxed">Engage users with automated notification triggers, clinical reminders, transactional dispatches, and real-time live chats mapped inside standard flows.</p>
                </div>

                <!-- Zoom & Transcribing -->
                <div class="p-8 border border-slate-100 bg-slate-50 rounded-2xl shadow-sm hover:shadow-lg transition">
                    <div class="w-12 h-12 bg-blue-100 text-blue-600 rounded-xl flex items-center justify-center mb-6">
                        <i class="fas fa-microphone-alt text-xl"></i>
                    </div>
                    <h3 class="font-extrabold text-lg text-slate-950 uppercase tracking-tight mb-2">Zoom Meeting Records</h3>
                    <p class="text-sm text-slate-500 leading-relaxed">Auto-sync recorded sales calls, meetings, or patient consulting video logs. Generate AI-driven textual transcriptions synced to contact profiles.</p>
                </div>

                <!-- Stripe Gateway -->
                <div class="p-8 border border-slate-100 bg-slate-50 rounded-2xl shadow-sm hover:shadow-lg transition">
                    <div class="w-12 h-12 bg-blue-100 text-blue-600 rounded-xl flex items-center justify-center mb-6">
                        <i class="fab fa-stripe-s text-xl"></i>
                    </div>
                    <h3 class="font-extrabold text-lg text-slate-950 uppercase tracking-tight mb-2">Stripe Invoicing</h3>
                    <p class="text-sm text-slate-500 leading-relaxed">Generate instant payment and booking deposit links inside cases or opportunities. Real-time auto-reconciliation of ledger records upon transaction success.</p>
                </div>

                <!-- Custom UI / UX -->
                <div class="p-8 border border-slate-100 bg-slate-50 rounded-2xl shadow-sm hover:shadow-lg transition">
                    <div class="w-12 h-12 bg-blue-100 text-blue-600 rounded-xl flex items-center justify-center mb-6">
                        <i class="fas fa-layer-group text-xl"></i>
                    </div>
                    <h3 class="font-extrabold text-lg text-slate-950 uppercase tracking-tight mb-2">Lightning UI Modules</h3>
                    <p class="text-sm text-slate-500 leading-relaxed">Eliminate visual clutter with custom Lightning Web Components (LWC). Clean dashboards built strictly to the operating workflows of your agents.</p>
                </div>
            </div>
        </div>
    </section>

    <!-- INTER-SERVICES TECHNICAL ARCHITECTURE SECTION -->
    <section class="py-24 bg-slate-900 text-white overflow-hidden">
        <div class="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
            <div>
                <span class="text-blue-400 font-bold uppercase tracking-widest text-[10px]">The Technical Advantage</span>
                <h2 class="text-white text-3xl md:text-4xl font-extrabold uppercase tracking-tighter leading-tight mt-2 mb-6">Unified Search Marketing & Enterprise CRM Engineering</h2>
                <p class="text-slate-400 leading-relaxed mb-6">At CSIC, we don't just "plug in" tools. We build the full conversion pipeline. Every Salesforce integration is backed by our world-class search engine optimization (SEO) and search marketing (SEM) framework.</p>
                <p class="text-slate-400 leading-relaxed">This means that when high-value leads are acquired through technical SEO, they bypass third-party delay traps, passing immediately and securely directly into your standard objects for processing.</p>
            </div>
            <div class="bg-slate-800 p-8 rounded-3xl border border-slate-700 shadow-2xl relative">
                <h3 class="font-extrabold uppercase tracking-wider text-xs text-blue-400 mb-6">CSIC Enterprise Data Pipeline</h3>
                <div class="space-y-6">
                    <div class="flex items-start gap-4">
                        <div class="w-8 h-8 rounded-full bg-blue-500/20 text-blue-400 flex items-center justify-center font-bold text-xs">1</div>
                        <div>
                            <h4 class="font-bold text-white">Technical Search Acquisition</h4>
                            <p class="text-xs text-slate-500">Google rank-optimized structure grabs high-intent enterprise users.</p>
                        </div>
                    </div>
                    <div class="flex items-start gap-4">
                        <div class="w-8 h-8 rounded-full bg-blue-500/20 text-blue-400 flex items-center justify-center font-bold text-xs">2</div>
                        <div>
                            <h4 class="font-bold text-white">Frictionless Landing Processing</h4>
                            <p class="text-xs text-slate-500">Friction-free, clean LCP-optimized WordPress forms capture verified inputs.</p>
                        </div>
                    </div>
                    <div class="flex items-start gap-4">
                        <div class="w-8 h-8 rounded-full bg-blue-500/20 text-blue-400 flex items-center justify-center font-bold text-xs">3</div>
                        <div>
                            <h4 class="font-bold text-white">API Cloud Routing Sync</h4>
                            <p class="text-xs text-slate-500">Secure pipeline matches records, dispatches instant WhatsApp alerts, and processes sales pipelines.</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <!-- FORM SECTION -->
    <section id="gap-analysis" class="py-24 bg-slate-900 border-t border-slate-800 text-white text-center">
        <div class="max-w-3xl mx-auto px-6">
            <h2 class="text-3xl md:text-4xl font-extrabold mb-6 uppercase tracking-tighter leading-tight">
               <span class="text-white-force">Schedule Your</span> <br><span class="text-blue-500">CRM Technical Audit</span>
            </h2>
            <p class="text-slate-sharp mb-12 max-w-lg mx-auto">We will walk you through your data architecture, identifying manual process loops and data leaks. Absolute zero obligation.</p>
            
            <div class="bg-white rounded-3xl shadow-2xl p-6 md:p-10 audit-form-container text-left border border-slate-800">
                <?php echo do_shortcode('[wpforms id="5477"]'); ?>
            </div>
            <p class="mt-8 text-[10px] text-slate-500 uppercase font-black tracking-[0.3em]">Direct On-Shore Architecture Specialist Consultation</p>
        </div>
    </section>
</main>

<?php get_footer(); ?>